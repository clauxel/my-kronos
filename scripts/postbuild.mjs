...
const origin = 'https://kronos.rest'
const siteName = 'Kronos AI'
const defaultTitle = 'Kronos AI | Financial K-Line Forecasting Workspace'
const defaultDescription =
  'Kronos AI turns financial K-line foundation models into a practical OHLCV forecasting workspace for price paths, volatility review, and quant rollout planning.'
...
await writeStaticPage('/pricing', {
  title: 'Kronos AI Pricing | Pro Annual Forecast Workspace',
  description: 'Compare Kronos AI Starter, Pro, and Scale plans. Pro annual is selected by default and annual billing is 50% cheaper.',
  robots: 'index,follow',
...
await writeStaticPage('/privacy', {
  ...
  description: 'How Kronos AI handles analytics, checkout metadata, and managed-site interactions.',
  robots: 'index,follow',
  canonicalPath: '/privacy',
  rootHtml: buildLegalPrerender('Privacy Policy', 'This policy covers analytics, checkout, and user interactions on the managed Kronos AI site.'),
  structuredData: [],
})

await writeStaticPage('/terms', {
  title: `Terms | ${siteName}`,
  description: 'Terms for using the Kronos AI managed site, hosted payment flow, and related support services.',
  robots: 'index,follow',
  canonicalPath: '/terms',
  rootHtml: buildLegalPrerender('Terms of Service', 'These terms describe the limits and responsibilities of the managed Kronos AI site.'),
  structuredData: [],
})

await writeStaticPage('/checkout/done', {
  title: `Checkout | ${siteName}`,
  description: 'Completing your Kronos AI checkout.',
  robots: 'noindex,nofollow',
  canonicalPath: '/checkout/done',
  rootHtml: buildLegalPrerender('Finishing checkout...', 'You will return to the Kronos AI homepage when the hosted payment session closes.'),
  structuredData: [],
})
...
function renderHtml({ title, description, robots, canonicalPath, rootHtml, structuredData }) {
  ...
  if (graph) {
    html = html.replace(
      '</head>',
      `    <script type="application/ld+json" id="kronos-prerender-schema">${JSON.stringify(graph)}</script>\n  </head>`,
    )
  }
...
function buildHomePrerender() {
  return `
    <main class="kro-main">
      <section class="kro-hero" id="workspace">
        <div>
          <p class="kro-eyebrow">Kronos AI forecasting workspace</p>
          <h1>Turn market K-lines into forecast decisions.</h1>
          <p class="kro-lede">Check OHLCV readiness, choose a forecast lane, and move into Pro annual checkout without leaving the page.</p>
          <p><a class="kro-btn kro-btn-primary" href="/pricing">Start Pro annual</a></p>
        </div>
      </section>
    </main>`
}
...
function buildPricingPrerender() {
  return `
    <main class="kro-main">
      <section class="kro-pricing-page-hero">
        <p class="kro-eyebrow">Pricing</p>
        <h1>Kronos AI pricing starts with the middle plan selected and annual billing already on.</h1>
        <p class="kro-lede">Pro annual is the default path and annual billing is 50% cheaper than the monthly run-rate.</p>
      </section>
    </main>`
}
...
function buildKeywordPrerender(page) {
  ...
  return `
    <main class="kro-main">
      <article class="kro-article">
        <a href="/">Kronos AI</a>
        <p class="kro-eyebrow">${escapeHtml(page.eyebrow)}</p>
        <h1>${escapeHtml(page.h1)}</h1>
        <p class="kro-lede">${escapeHtml(page.lede)}</p>
        ...
        <p><a class="kro-btn kro-btn-primary" href="/pricing">${escapeHtml(page.ctaLabel)}</a></p>
      </article>
    </main>`
}
  ...
  return `
    <main class="kro-main">
      <article class="kro-article">
        ...
        <p class="kro-lede">${escapeHtml(description)}</p>
      </article>
    </main>`
}
